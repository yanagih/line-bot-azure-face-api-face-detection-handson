...
async function handleEvent(event) {
  ...
  if (event.message.type === "image") {
    try {
      ...
      // 画像を取得し終わったらFace APIに送信して、結果（=レスポンス）を取得する
      const response = await faceClient.face.detectWithStream(image, {
        returnFaceLandmarks: true,
      });
      console.log(JSON.stringify(response));

      // レスポンスから、返信するメッセージを組み立てる
      let text = "";
      if (response.length === 0) {
        // 結果が0件の場合
        text += "送信された画像から顔を検出できませんでした。";
      } else {
        // 結果がある場合、最初の結果を表示する
        text += "顔を検出しました！\n";
        text += "\n";
        ...
        // 起きている/寝ているを判定する
        if (
          (response[0].faceLandmarks.eyeLeftBottom.y - response[0].faceLandmarks.eyeLeftTop.y) / response[0].faceRectangle.height > 0.04 && // 左目が開いている
          (response[0].faceLandmarks.eyeRightBottom.y - response[0].faceLandmarks.eyeRightTop.y) / response[0].faceRectangle.height > 0.04 // 右目が開いている
        ) {
          text += "起きています！";
        } else {
          text += "寝ていますzzz";
